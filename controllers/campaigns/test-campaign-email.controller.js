...
class TestCampaignController extends WebController {
  constructor() {
    super("/:campaignId/actions/test", HttpMethod.POST, [
  ...
  async handler(req, res) {
    try {
      ...
      await axios.post(
        `${api_endpoint}/${MAILCHIMP_API_VERSION}/campaigns/${campaignId}/actions/test`,
        null,
        {
          headers: {
      ...
      return res
        .status(200)
        .json(success(`Test email/s for campaign ${campaignId} has been sent`));
    } catch (err) {
      console.error(err);
...
module.exports = new TestCampaignController();
