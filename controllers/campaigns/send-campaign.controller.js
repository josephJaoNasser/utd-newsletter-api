...
class SendCampaignController extends WebController {
  constructor() {
    super("/:campaignId/actions/send", HttpMethod.POST, [
  ...
  async handler(req, res) {
    try {
      ...
      await axios.post(
        `${api_endpoint}/${MAILCHIMP_API_VERSION}/campaigns/${req.params.campaignId}/actions/send`,
        null,
        {
          headers: {
      ...
      return res
        .status(200)
        .json(success(`Campaign ${req.params.campaignId} has been sent!`));
    } catch (err) {
      console.error(err);
...
module.exports = new SendCampaignController();
